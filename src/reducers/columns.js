...
export  default function (columns = columnsInitialState, action) {
    switch (action.type) {
        ...
        case 'REMOVE_CARD':
            return removeCard(columns, action);
...
function removeCard(columns, action) {
    let columnId = store.getState().cards[action.payload].columnId;
    let newColumns = helpers.copyObject(columns);

    helpers.removeElementFromArray(newColumns[columnId].cards, action.payload);
